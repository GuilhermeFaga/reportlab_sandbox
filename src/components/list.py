...
class List(Flowable):
    ...
    def __init__(self, title: str, list_data: ListData, debug_flag: int = 0):
        self.title = title
    ...
    def wrap(self, aW, aH):
        ...
        self.list_primitive = ListPrimitive(self.list_data, debug_flag=self.debug_flag)

        self.title_primitive.wrapOn(self.canv, self.max_width, 1)
        self.list_primitive.wrapOn(self.canv, self.max_width, 1)

        self.height = (
            self.title_primitive.height + self.list_primitive.height + Spacing.Gap * 2
        )
